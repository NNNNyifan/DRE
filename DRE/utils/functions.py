...
def sentiment_data_filtering(sentiment_data, user_thresh, feature_thresh):
    ...
    #print("original feature length: ", len(features))
        ...
        #sentiment_data = feature_filtering(sentiment_data, valid_features)#清除不达标的feature
        #-----------------------------------------------------------------------------------
        ...
        for key, value in user_dict.items():
            if len(value) > (user_thresh) and len(value)< 9:
                valid_user.add(key)
    ...
    #return sentiment_data,feature_count_dict
...
def feature_filtering(sentiment_data, valid_features):
    ...
    for row in sentiment_data:
        ...
        item = row[1]
        text=row[2]
        cleaned_sentiment_data.append([user, item, text])
        
        ft=row[3:]
        for fos in ft:
            if fos in valid_features:
                cleaned_sentiment_data[-1].append(fos)
        # if len(cleaned_sentiment_data[-1]) == 2:#当没有评价的时候，删除该sentiment_data
        #     del cleaned_sentiment_data[-1]
    
    return cleaned_sentiment_data
    #return np.array(cleaned_sentiment_data)#使用np报错
